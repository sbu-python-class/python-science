...
class FitterGUI:
    def __init__(self, parent):
        ...
        self.frame_data = ttk.Frame(self.parent, relief=Tk.SUNKEN, borderwidth=1)
        self.frame_data.pack(fill=Tk.BOTH, expand=1, ipadx=5, ipady=5)
        
        self.dlabel = ttk.Label(self.frame_data, text="data")
        self.dlabel.grid(row=0, column=0, sticky=Tk.W, padx=5)

        self.file_label = ttk.Label(self.frame_data, text="file: ")
        self.file_label.grid(row=1, column=0, sticky=Tk.W, padx=5, ipadx=15)


        # fitting function
        
        self.frame_func = ttk.Frame(self.parent, relief=Tk.SUNKEN, borderwidth=1)
        self.frame_func.pack(fill=Tk.BOTH, expand=1, ipadx=5, ipady=5)
        
        self.flabel = ttk.Label(self.frame_func, text="fitting function")
        ...
        self.plot_button = ttk.Button(self.frame_buttons, text='Fit',
                                     command=self._plot)
        self.plot_button.pack(side=Tk.BOTTOM)

        self.clear_button = ttk.Button(self.frame_buttons, text='Clear',
                                      command=self._clear)
        self.clear_button.pack(side=Tk.BOTTOM)

        self.quit_button = ttk.Button(self.frame_buttons, text='Quit',
                                     command=self._quit)
